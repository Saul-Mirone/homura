...
type PostKeys = 'id' | 'title' | 'unread' | 'starred' | 'date' | 'sourceId' | 'link';

type PostItem = Pick<Source, SourceKeys> & Pick<Post, PostKeys>;
type PostList = Array<PostItem>;

export function getPostList(db: Database, options: GetPostListOptions): PostList {
    ...
    if (typeof options !== 'string') {
        return db.prepare<number>(selectPostsBySourceId(options.type)).all(options.id).map(transformer);
    }
    switch (options) {
        case Preset.Starred:
